...
def overfitting_plot(data):
    ...
    data['effective_loss'] = data['unbiased_loss']
    data.loc[data['eval-on'] == "noisy-test", 'effective_loss'] =  data.loc[data['eval-on'] == "noisy-test", 'unbiased_test_loss']
    ...
    for name, group in grouped:
        ...
        grid = sns.relplot(
            ...
            x="config/l2_reg", y=f"effective_loss",
            hue="Setting", style="eval-on",
            hue_order=hue_order,
            style_order=["noisy-test", "noisy-train", "noisy-val"],
            facet_kws={'sharey': False, 'sharex': False},
            linewidth=2.0, aspect=1.4, height=3.5, errorbar="sd"
        )
        grid.set(xscale="log")
        grid.set_titles(template="")
        grid.set_axis_labels("$L_2$", "loss")
        grid.set(xlim=(1e-7, 10))
        if name == "cce-norm":
            #grid.set(ylim=(-1.0, None))
            grid.set(xlim=(1e-6, 10))
            pass
        elif name == "bce-norm":
            grid.set(ylim=(-0.05, 0.15), xlim=(1e-6, 1e-0))
        elif name == "cce":
            grid.set(xlim=(1e-3, 1e3))
            #grid.set(ylim=(None, 11), xlim=(1e-6, 1))
        elif name == "bce":
            grid.set(ylim=(0.0, 0.4), xlim=(1e-4, 1e1))
...
def plot_at_k_for_paper(data, base_type):
    ...
    grouped = data.groupby("LossDecomposition")
    for name, group in grouped:
        Path(f"{name}-grp.txt").write_text(group.to_markdown())
        plot = sns.lineplot(
            data=group,
            x="config/l2_reg", y=f"noisy-test/value",
        ...
        if name.lower() == "bce":
            plot.set(xlim=(1e-4, 1))
        else:
            plot.set(xlim=(1e-3, 10000))
...
full_data = load_data(["/home/erik/Documents/latex/2024/JMLR-Missing/code/yahoo/yahoo.json"])
...
overfitting_plot(full_data)
#plot_loss_for_paper(full_data)
plot_at_k_for_paper(full_data, "tsPSP")
