...
def run_all_settings(index: int, losses: List[str]):
    train_ds = mlds.read_data("yahoo_train.txt")
    test_ds = mlds.read_data("yahoo_test.txt")
    train_prop = np.load("yahoo_train_prop.npy")
    test_prop = np.load("yahoo_test_prop.npy")

    data = generate_datasets(train_ds, test_ds, train_prop, test_prop)
    ...
    def rce(train_mode: str, loss_mode: str, normalized: bool, base_loss: str, pretraining: int = 0):
        ...
        regs = np.logspace(1.0, -4, 6, endpoint=True)
        regs = [100]
        if base_loss == "sqh":
            regs = np.logspace(1.4, -7+1.4, 11, endpoint=True)
        elif base_loss == "bce":
            regs = np.logspace(1, -4, 6, endpoint=True)
        elif base_loss == "cce":
            regs = np.logspace(3.0, -3, 7, endpoint=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--result-path", type=str, default="yahoo.json")
    parser.add_argument("--losses", type=str, default="cce,bce")
